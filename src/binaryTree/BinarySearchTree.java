...
public class BinarySearchTree {
    ...
    int findMaxRecursion(Node node) {
        if (node == null)
            return -1;
        int max = findMaxRecursion(node.right);
        return max < node.key ? node.key : max;
    }
    int findHeightRecursion(Node root) {
        if (root == null)
            return 0;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Recursive Max in BST new");
        int maxRec = bst.findMaxRecursion(root);
        System.out.println("Recursive Max: " + maxRec);
